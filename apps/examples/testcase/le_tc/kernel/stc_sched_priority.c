...
static void stc_sched_pri02_strict_ordering(void)
{
	...
	/* Worker 0 is the highest priority, descending from there, so the
	 * first ncpus entries are the ones that should run.
	 */
...
static void stc_sched_pri03_raise_waiting_task(void)
{
	...
	printf("[SCN-PRI-03] promoted worker %d count_after=%u\n",
	       target, (unsigned int)after);
...
static void stc_sched_pri04_lower_running_task(void)
{
	...
	/* And the demoted task must have given up ground, otherwise the
	 * peers could be running for some reason unrelated to the demotion.
	 */

	TC_ASSERT_LT("pri04_demoted_yielded", top_after, top_before);
